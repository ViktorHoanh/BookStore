...
//                str = str + "&";
        ...
        <main>
            ...
            <h2 class="text-uppercase text-center">${requestScope.name}</h2>
            <div class="container" style="margin-bottom: 3%;">
                ...
                <div class="pagination">   
                    <c:if test="${curPage!=1}">
                        <button class="img-circle" onclick='window.location = "search_tag?curPage=${1}&tag=${tag}&id=${id}&sort=${sort} "'><i class="fas fa-angle-double-left fa-2x"></i></button>
                        <button class="img-circle" onclick='window.location = "search_tag?curPage=${curPage-1}&tag=${tag}&id=${id}&sort=${sort}"'><i class="fas fa-angle-left fa-2x"></i></button>
                        </c:if>
                        ...
                        <a class="${i==curPage?"active":""}" href="search_tag?curPage=${i}&tag=${tag}&id=${id}&sort=${sort}">${i}</a>
                    </c:forEach>
                    <c:if test="${curPage!=requestScope.NoPage}">
                        <button class="img-circle" onclick='window.location = "search_tag?curPage=${curPage+1}&tag=${tag}&id=${id}&sort=${sort}"'><i class="fas fa-angle-right fa-2x"></i></button>
                        <button class="img-circle" onclick='window.location = "search_tag?curPage=${requestScope.NoPage}&tag=${tag}&id=${id}&sort=${sort}"'><i class="fas fa-angle-double-right fa-2x"></i></button>
                        </c:if>                   
                </div>
        ...
        <script>
            function myFunc(x) {
                var y = x.value;
                window.location = "search_tag?&tag=${tag}&id=${id}&sort=" + y;
            }
        </script>
    </body>
</html>
